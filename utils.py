...
def heading_to_world_frame(heading: int, theta_0: float):
    ...
    """
    return heading * math.pi / 4 + theta_0
...
def plot_path(ax, path, cost_map, ship):
    ...
    for i in range(np.shape(path)[0] - 1):
        p1 = path[i]
        p2 = path[i + 1]
        theta_0 = heading_to_world_frame(p1[2], ship.initial_heading) % (2 * math.pi)
        theta_1 = heading_to_world_frame(p2[2], ship.initial_heading) % (2 * math.pi)
